...
CREATE OR REPLACE FUNCTION add_menu_item(p_name TEXT, p_description TEXT, p_categoryone INT, p_categorytwo INT, p_categorythree INT, p_coffeetype INT, p_milkkind INT, p_price NUMERIC)
RETURNS VOID AS $$
BEGIN
    INSERT INTO MenuItem (Name, Description, CategoryOne, CategoryTwo, CategoryThree, CoffeeType, MilkKind, Price)
    VALUES (p_name, p_description, p_categoryone, p_categorytwo, p_categorythree, p_coffeetype, p_milkkind, p_price);
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION update_menu_item(p_menuitem_id INT, p_name TEXT, p_description TEXT, p_categoryone INT, p_categorytwo INT, p_categorythree INT, p_coffeetype INT, p_milkkind INT, p_price NUMERIC, p_reviewdate DATE, p_reviewer VARCHAR)
RETURNS VOID AS $$
BEGIN
    UPDATE MenuItem
    SET Name = p_name, Description = p_description, CategoryOne = p_categoryone, CategoryTwo = p_categorytwo, CategoryThree = p_categorythree, CoffeeType = p_coffeetype, MilkKind = p_milkkind, Price = p_price, ReviewDate = p_reviewdate, Reviewer = p_reviewer
    WHERE MenuItemID = p_menuitem_id;
END;
$$ LANGUAGE plpgsql;